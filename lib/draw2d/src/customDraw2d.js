...
draw2d.CustomCanvas = draw2d.Canvas.extend({
	...
	/**
     * @method
     * need to be called by outside to enable events of communications with outside
	...
	_selectEventHandler: function(emitter, event){
		if(event.figure!==null && event.figure.name === 'Connection'){
			...
			this.configPanel.fireEvent("fromCanvas", {
	...
	_fromConfigPanelEventHandler: function(canvas, eventObj){
		if(eventObj.type === 'setWeight')
		{
			this.selectedFigure.setWeight(eventObj.weight);
		}
		else
		{
			console.log("_fromConfigPanelEventHandler fired; unknown event type");
		}
	},
...
draw2d.CustomConnection = draw2d.Connection.extend({
	...
	/**
     * @method
     * setter for this.weight; change stroke accordingly
     */
	...
	/**
     * @method
     * set stroke according to weight
     */
	setStrokeFromWeight: function(){
		this.setStroke(this.weight * 2);
	}
});
