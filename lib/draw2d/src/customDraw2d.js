...
draw2d.CustomCanvas = draw2d.Canvas.extend({
	...
	init: function(id){
		...
		this.globalConfigPanel = null;		//initialize by calling setGlobalConfigPanel from outside
		this.selectedConfigPanel = null;	//initialize by calling setSelectedConfigPanel from outside
     ...
     * @param {Ext.Component} obj
     */
	setGlobalConfigPanel: function(obj){
		this.globalConfigPanel = obj;
	},
     ...
     * @param {Ext.Component} obj
     */
	setSelectedConfigPanel: function(obj){
		this.selectedConfigPanel = obj;
	},
     ...
     * need to have setSelectedConfigPanel() called from outside before any event happens
     */
	initEventHandlers: function(){
		//event 'fromSelectedConfigPanel' is used for receiving and handling events from outside config panel
	...
	_selectEventHandler: function(emitter, event){
		if(this.selectedConfigPanel)
		{
			if(event.figure!==null && event.figure.name === 'Connection')
			{
				...
				this.selectedConfigPanel.fireEvent("canvasSelect", {
	...
	getPersistentAttributes: function(){
		...
		//add layerRecs and nodeCirs into memento; take each element's ts
		memento.layerRecs = [];
		...
		memento.defaultWeight = this.defaultWeight;
		memento.defaultRouter = this.defaultRouter;
		memento.defaultLabelOn = this.defaultLabelOn;
...
draw2d.CustomConnection = draw2d.Connection.extend({
	...
	getPersistentAttributes: function(){
		...
		//source, target are in default memento
